...
addMotifExpression <- function(sco){
  ...
  #### ------- Original calculation using median as pseudo healthy cell -------------------------------------------------
  ...
  #### ------- Inserting precalculated log2 fold change scores directly -------------------------------------------------
  cat("extract rownames\n")  
  row_names <- rownames(sco$exp)
  cat("extracted rownames.      Example:",row_names[1],"    , should be like ENSG00000187634","\n", "getting foldchangescores","\n") 
  # ----- MORTENS METHOD ------
  #fold <- readRDS("/faststorage/project/jsp_student_projects/miRNA_DGD_DE_F2024/data/pseudocount_1e-6/morten_log2_fold_change_scores.rds") # MORTENS       pseudocount = 1e-6
  #fold <- readRDS("/faststorage/project/jsp_student_projects/miRNA_DGD_DE_F2024/data/pseudocount_1/morten_log2_fold_change_scores.rds") # MORTENS       pseudocount = 1

  # ---------- DGD -----------
  #fold <- readRDS("/faststorage/project/jsp_student_projects/miRNA_DGD_DE_F2024/data/pseudocount_1e-6/log2_fold_changes_filtered_normalized.rds") # DGD  pseudocount = 1e-6
  #fold <- readRDS("/faststorage/project/jsp_student_projects/miRNA_DGD_DE_F2024/data/pseudocount_1/log2_fold_changes_filtered_normalized.rds") # DGD    pseudocount = 1


  cat("filtering for samples in exp\n")  
  filtered <- fold[row_names, , drop = FALSE]
  cat("filtered mat dim: ", dim(filtered), "vs prefiltered dim: ", dim(fold)," removing ",nrow(filtered)-nrow(fold) ," genes \n")  
  cat("checking that the added genes which are filled with 0's are removed:",any(rowSums(filtered)==0)," should be FALSE\n")  
  var <- apply(filtered,2,function(x)order(x, decreasing = TRUE))
  cat("DGD log2 fold changes loaded, fitted and now running miReact \n")  
  #### ------------------------------------------------------------------------------------------------------------------
...
sink("sbatch.script")
cat("#!/bin/sh","#SBATCH --nodes 1","#SBATCH -c 1","#SBATCH --time 4:00:00",paste0("#SBATCH --account ",sco$runparameters$a),"#SBATCH --mem 16g","#SBATCH --job-name Rscript",sep="\n")
